...
func testUserOneToOneCatUsingOwnerCat(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, catDBTypes, true, catColumnsWithDefault...); err != nil {
	...
	foreign.OwnerID = local.ID
	if err := foreign.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.OwnerCat().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadOwnerCat(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OwnerCat == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.OwnerCat = nil
	if err = local.L.LoadOwnerCat(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OwnerCat == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpCatUsingOwnerCat(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, catDBTypes, false, strmangle.SetComplement(catPrimaryKeyColumns, catColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, catDBTypes, false, strmangle.SetComplement(catPrimaryKeyColumns, catColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Cat{&b, &c} {
		err = a.SetOwnerCat(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		zero := reflect.Zero(reflect.TypeOf(x.OwnerID))
		reflect.Indirect(reflect.ValueOf(&x.OwnerID)).Set(zero)
		...
		if a.ID != x.OwnerID {
			t.Error("foreign key was wrong value", a.ID, x.OwnerID)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `DiscordID`: `text`, `Money`: `bigint`, `LastDaily`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
